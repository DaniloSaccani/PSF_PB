...
if ENABLE_THETA0_SWEEP:
    ...
    for th0 in theta0_grid:
        ...
        if PLOT_LEARNED_IN_SWEEP:
            ...
            x_t = torch.tensor([th0, 0.0], dtype=torch.double).view(1,1,-1)
            # re-init MAD windows for this θ0
            mad.set_ep_initial_state(initial_state=x_t.view(1,-1))
            mad.reset_ep_timestep()
            mad.update_dynamics_input_time_window()
            mad.w = torch.zeros_like(x_t.view(-1))
            tl = [th0]; U_prev = X_prev = None
            for k in range(sim_horizon):
                x_np = x_t.view(-1).cpu().numpy()
                state_error = (x_t.view(1,-1) - mad.target_state).to(next(mad.actor_model.parameters()).device)
                uL_tensor = mad.learned_policy(
...
# (a) θ & ω vs time
ax_a = axes[0]
ax_a.plot(t_states, theta_base_err, lw=1.6, color=COLOR_BASELINE, label=r'$\tilde{\theta}$ baseline' if SHIFT_TO_ERROR_FRAME else r'$\theta$ baseline')
ax_a.plot(t_states, theta_learn_err, lw=1.6, color=COLOR_LEARNED,  label=r'$\tilde{\theta}$ learned'  if SHIFT_TO_ERROR_FRAME else r'$\theta$ learned')
